...
const InternshipSection = () => {
  ...
  const handleCardClick = (internshipTitle: string) => {
    // Redirect to find jobs page with internship filter
    navigate(`/find-jobs?type=internship&category=${encodeURIComponent(internshipTitle)}`);
  };
    ...
    <div className="py-16 bg-white">
      ...
      <Marquee speed={40} gradient={false} pauseOnHover={true} className="py-4">
        {internshipCategory.map((intern, index) => (
          <div
            key={index}
            onClick={() => handleCardClick(intern.title)}
            className="flex flex-col items-center justify-between
            bg-white w-64 h-72 
            border border-gray-200 rounded-xl 
            p-5 mx-4 shadow-md 
